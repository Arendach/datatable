...
const formatFilterValue = (column: Column): string => {
...
const filters = computed(() =>
  dataTableStore.columns
    .filter(col => col.filter.value !== null && col.filter.value !== '')
    .map(col => ({
      field: col.field,
      title: col.title,
...
function removeFilter(field: string) {
  const columnMap = new Map(dataTableStore.columns.map(col => [col.field, col]))
...
function clearAllFilters() {
  dataTableStore.columns.forEach(col => {
